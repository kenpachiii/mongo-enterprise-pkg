...
function MysqlFunctions(opts) {
  ...
  function createUser(username, options) {
    ...
    if (existsUser(username, options)) {
      throw new Error(`Cannot create the user ${username}. It already exists.`);
    }
    _.forEach(_.flatten(options.fromHosts), function(fromHost) {
      let command = `CREATE USER '${username}'@'${fromHost}'`;
      if (!_.isEmpty(options.newUserPassword)) command += ` IDENTIFIED BY '${options.newUserPassword}'`;
      execute(command, options);
    });
  }
  ...
  function grantPrivileges(database, username, options) {
    ...
    _.forEach(_.flatten(options.fromHosts), (fromHost) => {
      ...
      if (!_.isEmpty(options.databaseUserPassword)) command += ` IDENTIFIED BY '${options.databaseUserPassword}'`;
